...
This gate checks built HTML, including named HTML support pages. It is separate
from factual page quality, and both strict gates are required for publication.
An unreadable or empty tree is UNKNOWN, never a passing result.
...
def c_masthead(t: str) -> str:
    ...
    ok = len(re.findall(r"<header[\s>]", t)) == 1 and re.search(r'<header[^>]*class="[^"]*\bmasthead\b', t)
    return "" if ok else 'no <header class="masthead"> shell (BRAND.md §6)'


def c_footer(t: str) -> str:
    t = re.sub(r"<script\b[^>]*>.*?</script>", "", t, flags=re.S)
    ok = len(re.findall(r"<footer[\s>]", t)) == 1 and re.search(r'<footer[^>]*class="[^"]*\bsite\b', t)
...
def themes_verdict(css_path: Path) -> str:
    ...
    for token, value in (("primary-surface", "207 100% 42%"), ("primary-surface-hover", "207 100% 36%")):
        values = re.findall(r"--" + token + r"\s*:\s*([^;]+);", css)
        if len(values) != 3 or any(v.strip() != value for v in values):
            missing.append(token + " must match DOCX v1.3 in all three theme definitions")
    if missing:
